...
public class InteractionsManager extends AbstractNodeMain {
    private final static String TAG = "InteractionsM";
    ...
    public void shutdown() {
        if (nodeMainExecutorService != null)
            ...
            Log.i(TAG, "Shutting down an uninitialized apps manager");
    }
    ...
    public void getAppsForRole(final MasterId masterId, final String role) {
        ...
        if (this.connectedNode == null) {
            Log.i(TAG, "First action requested (" + this.action + "). Starting node...");
    ...
    private void getAppsForRole() {
        ...
        String serviceName = this.interactionsNamespace + "/get_interactions"; // /interactions/get_interactions

        try {
            Log.i(TAG, "List interactions service client created [" + serviceName + "]");
            srvClient = connectedNode.newServiceClient(serviceName, GetInteractions._TYPE);
        } catch (ServiceNotFoundException e) {
            Log.i(TAG, "List interactions service not found [" + serviceName + "]");
        ...
        request.getRoles().add(role); // role=null， ANDROID_PLATFORM_INFO.getUri()=rocon:/*/*/indigo/ice_cream_sandwich|jellybean|chrome
        request.setUri(ANDROID_PLATFORM_INFO.getUri());
        Log.i(TAG, "getAppsForRole: role=" + role + ", ANDROID_PLATFORM_INFO.getUri()=" + ANDROID_PLATFORM_INFO.getUri());

        try {
            srvClient.call(request, getAppsServiceResponseListener); // IllegalArgumentException
        } catch (Exception e) {
            e.printStackTrace();
        }
        Log.i(TAG, "List interactions service call done [" + serviceName + "]");
    }
    ...
    private void requestAppUse() {
        ...
        Log.i(TAG, "requestAppUse serviceName=" + serviceName);
        try {
            Log.i(TAG, "Request app service client created [" + serviceName + "]");
            srvClient = connectedNode.newServiceClient(serviceName, RequestInteraction._TYPE);
        } catch (ServiceNotFoundException e) {
            Log.i(TAG, "Request app service not found [" + serviceName + "]");
            throw new RosRuntimeException(e); // TODO we should recover from this calling onFailure on listener
        }
        ...
        Log.i(TAG, "Request app service call done [" + serviceName + "]");
    }
    ...
    private void getAppInfo() {
        ...
        try {
            Log.i(TAG, "Get app info service client created [" + serviceName + "]");
            srvClient = connectedNode.newServiceClient(serviceName, GetInteractions._TYPE);
        } catch (ServiceNotFoundException e) {
            Log.i(TAG, "Get app info not found [" + serviceName + "]");
            throw new RosRuntimeException(e); // TODO we should recover from this calling onFailure on listener
        }
        ...
        Log.i(TAG, "Get app info service call done [" + serviceName + "]");
    }
    ...
    private class ConnectNodeThread extends Thread {
        ...
        public void run() {
            try {
                ...
                Log.i(TAG, "invalid concert URI [" + masterId.getMasterUri() + "][" + e.toString() + "]");
                ...
                Log.i(TAG, "could not find concert [" + masterId.getMasterUri() + "][" + e.toString() + "]");
                failureCallback.handleFailure(e.toString());
            } catch (Throwable e) {
                Log.i(TAG, "exception while creating node in concert checker for URI " + masterId.getMasterUri(), e);
                failureCallback.handleFailure(e.toString());
            }
        }
    }
	...
	public void onStart(final ConnectedNode connectedNode) {
        if (this.connectedNode != null) {
            Log.e(TAG, "App manager re-started before previous shutdown; ignoring...");
            return;
        }
        ...
        switch (action) {
            case NONE:
                Log.i(TAG, "Node started without specifying an action");
                break;
            ...
            case GET_INTERACTION_INFO:
                ...
                break;
            default:
                Log.i(TAG, "Unrecognised action requested: " + action);
        }
    ...
    public void onShutdown(Node node) {
        Log.i(TAG, "Shutdown connected node...");
        super.onShutdown(node);

        // Required so we get reconnected the next time
        this.connectedNode = null;
        Log.i(TAG, "Done; shutdown apps manager node main");
    }
    ...
    public void onError(Node node, Throwable throwable) {
        ...
        Log.e(TAG, node.getName().toString() + " node error: " + throwable.getMessage());
