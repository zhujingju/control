...
//import static org.ros.android.android_15.R.id.previous_velocity_divet;
//import static org.ros.android.android_15.R.id.thumb_divet;
...
public class VirtualJoystickView extends RelativeLayout implements AnimationListener,
  ...
  private int mWhat = -1;
  public static enum enum_whatcode{unInit};
  private long mSendInterval = 2; // seconds
  private long mLastSend = -1;
...
//    Log.i(TAG, "onNewMessage: 收到数据"); // 不管能不能控制，只要init(nodeMainExecutor)之后，这里就一直在刷。
    double heading;
  ...
  public boolean onTouchEvent(MotionEvent event) {
    if(publisher == null) {
      Log.e(TAG, "onTouchEvent: 未初始化数据,mHandler="+mHandler+", mWhat="+mWhat);
      if(mHandler != null && mWhat != -1 && (Math.abs(System.currentTimeMillis() - mLastSend) / 1000 > mSendInterval)) {
        mHandler.obtainMessage(mWhat, enum_whatcode.unInit).sendToTarget(); // 两秒之内不重复
        mLastSend = System.currentTimeMillis();
        Log.i(TAG, "onTouchEvent: 已发送handler，数据未初始化");
      }
      return false;
    }
...
//    // ÉèÖÃ¿Ø¼þ´óÐ¡
    if (!isInited && mainLayout.getWidth() > 0 && mainLayout.getHeight() > 0) {
      Log.i(TAG, "onLayout: 看打印了几次，估计打印了很多次, mainLayout.w,h=(" + mainLayout.getWidth() + ", " + mainLayout.getHeight() + ")"); // 可刷屏mainLayout.w,h=(480, 480)
      resetVirtualJoystick(Math.min(mainLayout.getWidth(), mainLayout.getHeight()));
    }
  }
...
//    Log.i(TAG, "initVirtualJoystick: mainLayout.w,h=(" + mainLayout.getWidth() + ", " + mainLayout.getHeight() + ")"); // 0,0
//    if (mainLayout.getWidth() > 0 && mainLayout.getHeight() > 0) {
//      Log.i(TAG, "onLayout: 看打印了几次，估计打印了很多次"); //
//      resetVirtualJoystick(Math.min(mainLayout.getWidth(), mainLayout.getHeight()));
//    }
  }

  /**
   * ÖØÐÂÉèÖÃÒ»Ð©¿Ø¼þµÄ´óÐ¡
   */
  private void resetVirtualJoystick(int rootWidth) {
//    Log.i(TAG, "resetVirtualJoystick()");

    /** Ä£°å¿í¸ß£¬ºáÆÁ£¬Ô­Ê¼³ß´ç 1280*720, 1280/720=1.777777777777778,  2048/1536=1.333333333333333 */
    int mModuleDivetWidth = 80; // ´ø¼ýÍ·µÄÐ¡Ô²È¦ÔÚ1280*720µÄÆÁÄ»ÏÂÊÇ80*80
    int mModuleRootWidth = 300; // Õû¸öÊÖ±ú¿Ø¼þÔÚ1280*720µÄÆÁÄ»ÏÂÊÇ300*300


//    int x = rootWidth * mModuleDivetWidth / mModuleRootWidth; // 300 / 80 = rootWidth / x;
    RelativeLayout.LayoutParams lpThumb = (RelativeLayout.LayoutParams)thumbDivet.getLayoutParams();
    lpThumb.width =  rootWidth * mModuleDivetWidth / mModuleRootWidth; // 300 / 80 = rootWidth / x;
    lpThumb.height = lpThumb.width;
    thumbDivet.setLayoutParams(lpThumb);

    RelativeLayout.LayoutParams lpVelocityDivet = (RelativeLayout.LayoutParams)lastVelocityDivet.getLayoutParams();
    lpVelocityDivet.width =  lpThumb.width;
    lpVelocityDivet.height = lpVelocityDivet.width;
    lastVelocityDivet.setLayoutParams(lpVelocityDivet);

//    Log.i(TAG, "resetVirtualJoystick: lpThumb.width=" + lpThumb.width + ", lpVelocityDivet.width=" + lpVelocityDivet.width); // Ë¢ÆÁ lpThumb.width=128, lpVelocityDivet.width=128
    isInited = true;
  }
  ...
  private void onContactMove(float x, float y) {
    ...
    // sxg start
    x = x + (thisView.getWidth() / 2 - x) * 2 / 3;
    y = y + (thisView.getHeight() / 2 - y) * 2 / 3;
    // sxg end
    ...
    if (contactRadius >= 1f) {
      ...
      // The magnitude should not xceed 1.e
  ...
  private void onContactUp() {
    ...
    if(publisher == null || currentVelocityCommand == null){
      Log.e(TAG, "onContactUp: publisher is null, or currentVelocityCommand is null");
    }else {
  ...
  private void publishVelocity(double linearVelocityX, double linearVelocityY,
    ...
    if(currentVelocityCommand == null){
      Log.e(TAG, "publishVelocity: 未初始化成功，currentVelocityCommand is null");
      return;
    }
...
//    Log.i(TAG, "publishVelocity: linearVelocityX="+linearVelocityX+", linearVelocityY="+linearVelocityY+", angularVelocityZ="+angularVelocityZ);
  }
  ...
  protected void onDetachedFromWindow() {
    ...
    // 想在这里发个0速度，在失误的时候让机器人停止下来。
    publishVelocity(0, 0, 0);
    Log.i(TAG, "onDetachedFromWindow: 已发布停止速度");
  }
  ...
  /** 用来获取控件的当前宽高 */
  private VirtualJoystickView thisView;
