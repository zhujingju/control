...
public class SpriteBatch {
   private static final String TAG = "Print-SpriteBatch";
   //--Constants--//
...
//            sb.append(vertexBuffer[i] + ", ");
//         }
//         Log.i(TAG, "sb.toString()=" + sb.toString() + ", vertexBuffer.length=" + vertexBuffer.length); // 会一直刷屏
...
//         Log.i(TAG, "endBatch: numSprites=" + numSprites); // numSprites = 2  // 会一直刷屏
//         Log.i(TAG, "endBatch - vertices.draw");
         vertices.unbind();                           // Unbind Vertices
      }
   }


   public void endBatchBeforePush() {
      if ( numSprites > 0 )  {                        // IF Any Sprites to Render
         vertices.setVertices(vertexBuffer, 0, bufferIndex);  // Set Vertices from Buffer
         vertices.bind();                             // Bind Vertices
      }
   ...
   // R: [none]
   public void drawSprite(float x, float y, float width, float height, TextureRegion region, double zoom)  {
//      Log.i(TAG, "numSprites=" +numSprites+",maxSprites="+maxSprites+",x="+x+",y="+y+",width="+width+",height"+height+",region="+region.toString() +",bufferIndex="+bufferIndex);
...
//      org.ros.android.view.visualization.Vertices.drawLineLoop(gl, org.ros.android.view.visualization.Vertices.toFloatBuffer(vertexBuffer), Color.fromHexAndAlpha("ff5913", 0.2f), 0.5f);
//      System.out.print(TAG + ", vertexBuffer value:");
//      for(int i = 0; i < vertexBuffer.length; i++){
//         System.out.print(vertexBuffer[i] + ", ");
//      }
   }
}
